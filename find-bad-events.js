...
async function processFile(filePath) {
  return new Promise((resolve, reject) => {
    ...
    lr.on('error', function (err) {
      console.error(`Error while processing ${filePath}`, err);
      reject(err);
    });
    ...
    lr.on('end', async function () {
      ...
      rl.write(`${numberOfLines}-${Object.keys(badSenderMap).length}-${badEventIds.length}.`);

      try {
        await fs.appendFile(badSendersFilePath, Object.keys(badSenderMap).join('\n'));
      } catch (err) {
      ...
      try {
        await fs.appendFile(badEventIdsFilePath, badEventIds.join('\n'));
      } catch (err) {
